...
export default function ScenarioPanel({
  ...
  return (
    <>
      {/* Narration bar - positioned at TOP of viewport to avoid overlapping nodes */}
      <AnimatePresence>
        {isPlaying && narration && (
          <motion.div
            className="fixed top-14 left-1/2 -translate-x-1/2 z-50 px-4 w-full max-w-2xl"
            initial={{ y: -20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            exit={{ y: -20, opacity: 0 }}
          >
            <div className="bg-zinc-900/95 backdrop-blur-md border border-zinc-700 rounded-xl px-5 py-3 text-center shadow-lg shadow-black/30">
      ...
      {/* Scenario buttons - fixed at bottom */}
