...
try:
    from .video_build_mesh import attach_handoff_report, attach_to_render_spec, write_hyperframes_handoff
except Exception:
    try:
        from engines.video_build_mesh import attach_handoff_report, attach_to_render_spec, write_hyperframes_handoff
    except Exception:
        def attach_handoff_report(render_spec: dict | None, handoff_report: dict | None) -> dict:
            return dict(render_spec or {})

        def attach_to_render_spec(render_spec: dict | None, *, scene_manifest: dict | None = None) -> dict:
            return dict(render_spec or {})

        def write_hyperframes_handoff(*args, **kwargs) -> dict:
            return {'ok': False, 'status': 'unavailable', 'error': 'video_build_mesh unavailable'}

try:
    from .hyperframes_render_engine import render_composition as _hf_render, hyperframes_available
except Exception:
    try:
        from engines.hyperframes_render_engine import render_composition as _hf_render, hyperframes_available
    except Exception:
        def _hf_render(*args, **kwargs):
            return {'ok': False, 'error': 'hyperframes_render_engine unavailable'}

        def hyperframes_available():
            return False

try:
    from .render_judge import pick_best_output
except Exception:
    try:
        from engines.render_judge import pick_best_output
    except Exception:
        def pick_best_output(*args, **kwargs):
            return {'ok': False, 'error': 'render_judge unavailable'}
...
def _call_byteplus_ark(prompt: str, model: str = '', timeout: int = 120) -> str:
    """BytePlus ModelArk — OpenAI-compatible chat API (optional paid lane)."""
    try:
        from . import byteplus_ark as _ark
    except Exception:
        try:
            import engines.byteplus_ark as _ark
        except Exception:
            return '_(BytePlus Ark import error)_'
    if not _ark.is_configured():
        return '_(BytePlus Ark key missing)_'
    result = _ark.chat_completions(
        [{'role': 'user', 'content': prompt}],
        model=model or _ark.ark_model(),
        temperature=0.7,
        max_tokens=4096,
        timeout_s=timeout,
    )
    ...
    return f'_(BytePlus Ark error: {result.get("error", "unknown")})_'
...
def draft_render_spec(hermes: dict, outline: dict, mode: str = 'fast',
    ...
    if mode == 'deep' and _key('PERPLEXITY_API_KEY', 'DLS_PERPLEXITY_API_KEY'):
        ...
        text = _call_byteplus_ark(payload)
    else:
        text = _call_ollama(payload, timeout=240)
...
# ---------------------------------------------------------------------------
# HyperFrames parallel render execution
# ---------------------------------------------------------------------------

def execute_hyperframes_render(
    ...
    *,
    ...
    """Render the HyperFrames handoff composition to MP4.

    This runs after write_hyperframes_handoff() produces index.html.
    It muxes audio and optionally burns subtitles via FFmpeg after
    the HyperFrames render completes.
    """
    started = time.time()
    if not hyperframes_available():
        return {'ok': False, 'stage': 'check', 'error': 'HyperFrames CLI not available'}

    hf_handoff_dir = PROJECT_ROOT / 'out' / 'hyperframes-handoff'
    if not (hf_handoff_dir / 'index.html').exists():
        # Try to generate handoff if missing
        try:
            handoff = write_hyperframes_handoff(
                scene_manifest, render_spec, audio_spec, subtitle_spec,
                output_dir=hf_handoff_dir,
            )
            if not handoff.get('ok'):
                return {'ok': False, 'stage': 'handoff', 'error': handoff.get('error', 'handoff failed')}
        except Exception as e:
            return {'ok': False, 'stage': 'handoff', 'error': f'{type(e).__name__}: {e}'}

    # Run HyperFrames render
    hf_result = _hf_render(
        hf_handoff_dir,
        output_path=hf_handoff_dir / 'render.mp4',
        timeout=timeout,
    )
    if not hf_result.get('ok'):
        hf_result['elapsed_seconds'] = round(time.time() - started, 1)
        return hf_result

    # Post-process: mux audio + burn subtitles
    hf_raw = Path(hf_result['output_file'])
    out_path = hf_raw
    audio_file = _audio_file_from_spec(audio_spec or {})
    subtitle_file = _subtitle_file_from_spec(subtitle_spec or {}) if subtitle_spec else None
    ffmpeg = shutil.which('ffmpeg') or shutil.which('ffmpeg.exe')
    stages = list(hf_result.get('stages', []))

    if audio_file and ffmpeg:
        muxed = hf_raw.with_name(hf_raw.stem + '_muxed.mp4')
        mux_cmd = [
            ffmpeg, '-y', '-i', str(hf_raw), '-i', str(audio_file),
            '-c:v', 'copy', '-c:a', 'aac', '-b:a', '192k', '-shortest',
            str(muxed),
        ]
        mux_result = _run_subprocess(mux_cmd, 'mux_audio', timeout)
    ...
    if subtitle_file and ffmpeg:
        final = hf_raw.with_name(hf_raw.stem + '_final.mp4')
        sub_cmd = [
    ...
    if out_path != hf_raw and out_path.exists():
        # Replace the raw render with the final muxed/burned version
        hf_result['output_file'] = str(out_path)
        hf_result['bytes'] = out_path.stat().st_size

    hf_result['stages'] = stages
    hf_result['elapsed_seconds'] = round(time.time() - started, 1)
    return hf_result


# ---------------------------------------------------------------------------
# Parallel render execution — Remotion + HyperFrames
# ---------------------------------------------------------------------------

def execute_parallel_render(
    scene_manifest: dict | None = None,
    render_spec: dict | None = None,
    audio_spec: dict | None = None,
    subtitle_spec: dict | None = None,
    ...
    *,
    ...
    """Run Remotion and HyperFrames renders in parallel, then pick the best output.

    Returns a unified result dict with winner info and both engine results.
    """
    import concurrent.futures
    started = time.time()

    def _run_remotion():
        return execute_render(render_spec or {}, scene_manifest, audio_spec, subtitle_spec, subtitles_enabled, timeout=timeout)

    def _run_hf():
        return execute_hyperframes_render(scene_manifest, render_spec, audio_spec, subtitle_spec, timeout=timeout)

    remotion_result: dict = {'ok': False, 'error': 'not started'}
    hf_result: dict = {'ok': False, 'error': 'not started'}

    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
        fut_re = pool.submit(_run_remotion)
        fut_hf = pool.submit(_run_hf)

        for fut in concurrent.futures.as_completed([fut_re, fut_hf]):
            try:
                result = fut.result()
                if fut is fut_re:
                    remotion_result = result
                else:
                    hf_result = result
            except Exception as e:
                if fut is fut_re:
                    remotion_result = {'ok': False, 'error': f'{type(e).__name__}: {e}'}
                else:
                    hf_result = {'ok': False, 'error': f'{type(e).__name__}: {e}'}

    # Judge the outputs
    expected_duration = None
    if isinstance(scene_manifest, dict):
        expected_duration = float(scene_manifest.get('length_seconds') or scene_manifest.get('target_seconds') or 0) or None

    judge = pick_best_output(
        remotion_output=remotion_result.get('output_file'),
        hyperframes_output=hf_result.get('output_file'),
        expected_duration_s=expected_duration,
    )

    winner = judge.get('winner')
    winner_file = winner.get('output_file') if winner else None
    winner_engine = winner.get('engine') if winner else None

    # Copy winner to canonical output path
    canonical = PROJECT_ROOT / 'out' / 'final.mp4'
    if winner_file:
        try:
            canonical.parent.mkdir(parents=True, exist_ok=True)
            shutil.copyfile(winner_file, canonical)
        except Exception:
            pass

    return {
        'ok': bool(winner_file and Path(winner_file).exists()),
...
def run_step7(scene_manifest: dict | None = None, audio_spec: dict | None = None,
    ...
    render_spec = attach_to_render_spec(render_spec, scene_manifest=scene_manifest)
    ...
    # Generate the concrete HyperFrames handoff every time Step 7 builds a render
    # spec. Remotion remains the legacy executor, but new video work now has a
    # runnable HTML composition with lint evidence attached to the same artifact.
    t = time.time()
    hyperframes_report = write_hyperframes_handoff(
        scene_manifest=scene_manifest,
    ...
    asset_reports['hyperframes_handoff'] = hyperframes_report
    render_spec = attach_handoff_report(render_spec, hyperframes_report)
    stage_times['hyperframes_handoff'] = round(time.time() - t, 1)
