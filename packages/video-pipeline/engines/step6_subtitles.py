...
try:
    from .local_runtime import ensure_local_tools_on_path, whisper_model_dirs, which as _which
except Exception:
    ensure_local_tools_on_path = lambda: os.environ.get('PATH', '')
    whisper_model_dirs = lambda: [
...
def harvest_subtitle_tools() -> list[dict]:
    tools = [
        ('whisper-cli',    'transcription', 'Whisper.cpp CLI — bundled local transcription + SRT export'),
...
def harvest_whisper_models() -> list[str]:
    ...
    search_dirs = whisper_model_dirs()
    models = []
    seen = set()
    for d in search_dirs:
        if not d.exists():
            continue
        for f in d.iterdir():
            if f.suffix in ('.bin', '.pt', '.onnx') and f.is_file() and f.name not in seen:
                seen.add(f.name)
