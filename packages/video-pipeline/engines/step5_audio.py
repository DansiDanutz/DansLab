...
try:
    from .local_runtime import command_works, ensure_local_tools_on_path, piper_voice_dirs, which as _which
except Exception:
    command_works = lambda command, *args, timeout=5: bool(command)
    ensure_local_tools_on_path = lambda: os.environ.get('PATH', '')
    piper_voice_dirs = lambda: [
...
def harvest_tts_tools() -> list[dict]:
    ...
    for bin_name, category, desc in tools:
        ...
        if bin_name in ('piper', 'espeak-ng') and not command_works(path, '--help'):
            continue
        if path:
