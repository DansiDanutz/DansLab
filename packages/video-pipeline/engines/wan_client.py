...
Required local model files:
  models/diffusion_models/Wan2.2-TI2V-5B-Q4_K_M.gguf
...
from .comfyui_paths import comfyui_api_base, comfyui_models_dir
...
def _model_dir(subdir: str) -> Path:
    return comfyui_models_dir() / subdir
...
def is_configured() -> bool:
    """Wan local needs ComfyUI HTTP API + all 3 model files (unet + vae + text encoder)."""
...
def status() -> dict:
    ...
    note = ''
    if not _comfyui_reachable():
        note = 'requires ComfyUI HTTP API (set COMFYUI_HOST / COMFYUI_PORT)'
    elif missing:
