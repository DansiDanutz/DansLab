...
from .comfyui_paths import comfyui_api_base
...
#   1. ComfyUI — local API workflow, defaulting to verified SD 1.5 on :8000
#   2. Comfy Cloud — explicit opt-in remote OSS lane
#   3. Optional paid/free legacy providers, only when explicitly configured
...
COMFYUI_HOST = comfyui_api_base()
COMFYUI_WORKFLOW_PATH = os.environ.get(
    'COMFYUI_WORKFLOW',
    str(Path(__file__).resolve().parents[2] / 'comfyui-workflows' / 'sd15-base-frames-api.json'),
)
...
def _comfyui_inject_prompt(workflow: dict, prompt: str) -> dict:
    """Walk the ComfyUI API workflow JSON and replace the first positive prompt."""
...
def _comfyui_set_dimensions(workflow: dict, width: int, height: int) -> dict:
    """Apply requested dimensions to common latent image/video nodes."""
    for node in workflow.values():
        if not isinstance(node, dict):
            continue
        cls = node.get('class_type') or ''
        inputs = node.get('inputs', {})
        if cls in ('EmptyLatentImage', 'EmptyHunyuanLatentVideo') and isinstance(inputs, dict):
            if 'width' in inputs:
                inputs['width'] = width
            if 'height' in inputs:
...
def _comfyui_set_filename_prefix(workflow: dict, out_path: str) -> dict:
    """Use a stable Comfy output prefix for easier debugging without relying on it."""
    stem = Path(out_path).stem or 'scene'
    prefix = f'yt_pipeline/{stem}'
    for node in workflow.values():
        if not isinstance(node, dict):
            continue
...
def fetch_comfyui(prompt: str, out_path: str, width: int = DEFAULT_W,
    ...
    try:
        ...
        workflow = _comfyui_set_dimensions(workflow, width, height)
        workflow = _comfyui_set_filename_prefix(workflow, out_path)

        body = json.dumps({'prompt': workflow, 'client_id': 'youtube-pipeline'}).encode('utf-8')
        req = urllib.request.Request(
...
def video_provider_status() -> dict:
    ...
    seedance_block: dict = {
        'available': False,
        'note': 'set SEEDANCE_API_URL + key, or SEEDANCE_USE_MODELARK=1 + BYTEPLUS/ARK key',
    }
    higgs_ok = False
    higgs_note = ''
    fal_ok = False
    ...
    try:
        ...
        st = _sd.status()
        seedance_block = {
            'available': seedance_ok,
            'configured': st.get('configured', seedance_ok),
            'note': '' if seedance_ok else seedance_block['note'],
            'mode': st.get('mode'),
            'model': st.get('model'),
            'url': st.get('url'),
            'api_base': st.get('api_base'),
        }
    except Exception:
        pass
    try:
    ...
    try:
        ...
        pass
    legacy_comfy_i2v = (
        bool(os.environ.get('COMFYUI_I2V_WORKFLOW'))
        and _comfyui_reachable()
        and Path(COMFYUI_I2V_WORKFLOW_PATH).exists()
    )
    return {
        'comfyui':      {'available': legacy_comfy_i2v, 'host': COMFYUI_HOST,
                         'note': 'legacy I2V workflow is opt-in via COMFYUI_I2V_WORKFLOW'},
        'comfy-cloud':  cloud_status,
        'wan':          wan_status,
        'seedance':     seedance_block,
        'higgsfield':   {'available': higgs_ok, 'note': higgs_note},
        'fal':          {'available': fal_ok, 'note': '' if fal_ok else 'set FAL_API_KEY'},
...
DEFAULT_VIDEO_CHAIN: list[str] = []  # Auto-filled by configured OSS lanes; local-ffmpeg is the hard fallback.
...
def _video_provider_chain() -> list[str]:
    ...
    # Default chain. Add only lanes whose configuration gates are satisfied:
    #   1. Wan 2.2 local (ComfyUI :8000 + all 3 model files present)
    #   2. Comfy Cloud LTX-Video (ZMARTY_USE_COMFY_CLOUD=1 + key)
    #   3. Legacy generic ComfyUI I2V only when COMFYUI_I2V_WORKFLOW is explicit
    # Step 7 appends local ffmpeg motion as the no-surprise OSS fallback.
    ...
    try:
        ...
        if _cc.is_configured() and 'comfy-cloud' not in chain:
            chain.append('comfy-cloud')
    except Exception:
        pass
    try:
        from . import seedance as _sd
        if _sd.is_configured() and 'seedance' not in chain:
            chain.append('seedance')
    except Exception:
        pass
    if (
        os.environ.get('COMFYUI_I2V_WORKFLOW')
        and _comfyui_reachable()
        and Path(COMFYUI_I2V_WORKFLOW_PATH).exists()
        and 'comfyui' not in chain
    ):
        chain.append('comfyui')
    return chain
...
def fetch_video_with_fallback(image_path: str, prompt: str, out_path: str,
    ...
    for name in chain:
        try:
            if name == 'wan':
                from . import wan_client as _wan
                result = _wan.text_to_video(prompt, out_path, timeout_s=1800)
            elif name == 'comfyui':
