...
try:
    from .video_build_mesh import mesh_context, mesh_qa_checks
except Exception:
    try:
        from engines.video_build_mesh import mesh_context, mesh_qa_checks
    except Exception:
        def mesh_context(stage: str) -> dict:
            return {'stage': stage, 'version': 'unavailable'}

        def mesh_qa_checks(render_spec: dict | None, scene_manifest: dict | None) -> dict:
            return {'version': 'unavailable', 'checks': []}

try:
    from .design_agent_orchestrator import available_skills as _design_skills
except Exception:
    try:
        from engines.design_agent_orchestrator import available_skills as _design_skills
    except Exception:
        def _design_skills():
            return {}
...
def validate_qa_plan(hermes: dict, outline: dict, qa_plan: dict, render_spec: dict,
    ...
    # Design critique check (Phase 5)
    design_critique = scene_manifest.get('design_system', {}).get('orchestrator_critique') if scene_manifest else None
    if design_critique:
        score = design_critique.get('overall_score', 0)
        grades['design_critique_grade'] = 'GREEN' if score >= 7 else ('YELLOW' if score >= 5 else 'RED')
    else:
        grades['design_critique_grade'] = 'YELLOW'  # optional but encouraged
...
def run_step8(
    ...
    qa_plan['video_build_mesh'] = mesh_context('step8_qa')
    mesh_checks = mesh_qa_checks(render_spec, scene_manifest)
