...
from .comfyui_paths import comfyui_api_base, vibevoice_models_parent
...
def is_configured() -> bool:
    ...
    if not _comfyui_reachable():
        return False
    model_dir = vibevoice_models_parent() / VIBEVOICE_DEFAULT_MODEL
...
def status() -> dict:
    ...
    return {
        ...
        'note':        '' if has_model else ('requires ComfyUI + VibeVoice node + model under '
                           f'{vibevoice_models_parent() / VIBEVOICE_DEFAULT_MODEL}'),
    }
...
def synthesize(text: str, output_wav: str | Path,
    ...
    try:
        if not _comfyui_reachable():
            return {'ok': False, 'error': 'ComfyUI API unreachable (check COMFYUI_HOST)', 'duration_s': 0.0}
