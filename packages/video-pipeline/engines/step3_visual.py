...
try:
    from .video_build_mesh import attach_to_design_system
except Exception:
    try:
        from engines.video_build_mesh import attach_to_design_system
    except Exception:
        def attach_to_design_system(design_system: dict | None, *, notes: str = '') -> dict:
            return dict(design_system or {})

try:
    from .open_design_agent import generate_design_system as _od_generate_design_system, available as _od_available
except Exception:
    try:
        from engines.open_design_agent import generate_design_system as _od_generate_design_system, available as _od_available
    except Exception:
        def _od_generate_design_system(*args, **kwargs):
            return {}
...
try:
    from .design_agent_orchestrator import orchestrate as _design_orchestrate
except Exception:
    try:
        from engines.design_agent_orchestrator import orchestrate as _design_orchestrate
    except Exception:
        def _design_orchestrate(*args, **kwargs):
            return {'ok': False, 'error': 'design_agent_orchestrator unavailable'}
...
def _call_byteplus_ark(prompt: str, model: str = '', timeout: int = 240) -> str:
...
def run_step3(step1_brief: str = '', step2_script: str = '', mode: str = 'fast',
    ...
    # Open Design agent enrichment (Phase 2)
    if _od_available():
        try:
            od_design = _od_generate_design_system(
                {'topic': step1_brief, 'mood': hermes.get('mood', 'professional')},
                topic=step1_brief[:80] if step1_brief else '',
                platform='youtube',
            )
            if od_design:
                # Merge Open Design palette/motion into existing design
                design['open_design_enrichment'] = od_design
                if od_design.get('palette'):
                    design['color_palette'] = od_design['palette']
                if od_design.get('typography'):
                    design['typography'] = od_design['typography']
                if od_design.get('motion'):
                    design['motion_principles'] = od_design['motion']
                stage_times['open_design'] = round(time.time() - t, 1)
        except Exception:
            pass

    # Design agent orchestrator auto-critique (Phase 3)
    auto_design = (os.environ.get('AUTO_DESIGN_AGENTS') or 'false').strip().lower() not in {'0', 'false', 'no', 'off'}
    if auto_design:
        try:
            orch = _design_orchestrate(
                {'topic': step1_brief, 'mood': hermes.get('mood', 'professional')},
                topic=step1_brief[:80] if step1_brief else '',
                notes=notes,
            )
            if orch.get('ok'):
                ...
                design['orchestrator_critique'] = orch.get('skill_results', {}).get('critique', {})
                stage_times['design_orchestrator'] = round(time.time() - t, 1)
        except Exception:
            pass

    design = attach_to_design_system(design, notes=notes)
    ...
    while convergence_passes <= max_convergence:
        ...
        if revised:
            design = attach_to_design_system(revised, notes=notes)
            validators = validate_design(design)
