...
try:
    from .video_build_mesh import attach_to_scene_manifest
except Exception:
    try:
        from engines.video_build_mesh import attach_to_scene_manifest
    except Exception:
...
def draft_scene_manifest(hermes: dict, outline: dict, design_system: dict,
    ...
    if mode == 'deep' and _key('PERPLEXITY_API_KEY', 'DLS_PERPLEXITY_API_KEY'):
        ...
        text = _call_ollama(payload, timeout=300)
...
def run_step4(step2_script: str = '', design_system: dict | None = None,
    ...
    manifest = attach_to_scene_manifest(manifest, design_system=design_system)
