    ...
    *,
    ...
    if is_configured():
        try:
            ...
            from budget import preflight_charge, record_charge, LANE_DEFAULT_CREDITS

            est = estimated_credits
            if est is None:
                est = LANE_DEFAULT_CREDITS.get(lane, 6)
            gate = preflight_charge(project, lane, est)
            if not gate.get('allowed'):
                raise RuntimeError(gate.get('reason', 'budget cap exceeded'))
        except RuntimeError:
            raise
        except Exception:
            pass
    ...
    if is_configured():
        try:
            ...
            if str(dash) not in sys.path:
                sys.path.insert(0, str(dash))
            from budget import record_charge, LANE_DEFAULT_CREDITS

            est = estimated_credits if estimated_credits is not None else LANE_DEFAULT_CREDITS.get(lane, 6)
            record_charge(project, lane, est, notes=f'comfy-cloud prompt_id={pid}')
        except Exception:
            pass
    return pid
