...
| **LLM (BytePlus Ark)** | ModelArk OpenAI-compatible API | commercial | opt-in | `BYTEPLUS_API_KEY` + default chat model `seed-2-0-lite-260228` |
...
| `video_build_mesh.py` | Connects UI/UX Pro Max, Open Design, HyperFrames, Remotion compatibility, and QA metadata across Steps 3/4/7/8 | ✅ |
| `comfyui_paths.py` | Resolve `COMFYUI_HOST` / `COMFYUI_PORT` / default `models/` tree for Wan + VibeVoice | ✅ |
...
| `seedance.py` | optional BytePlus ModelArk video lane; invoked when configured |
