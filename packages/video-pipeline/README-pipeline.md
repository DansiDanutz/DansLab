...
- ✅ Pipeline doctor verifies ComfyUI reachability, optional `comfy-doctor`, image smoke, optional VibeVoice smoke, local tools, queue health
- ✅ Remotion build renders `src/index.tsx` → `out/video.mp4`
- ✅ Video Build Mesh connects UI/UX Pro Max, Open Design, HyperFrames, Remotion legacy rendering, and QA metadata across Steps 3, 4, 7, and 8

**Verified local defaults (May 2026):**
- Image chain → local ComfyUI on `http://127.0.0.1:8000` with `comfyui-workflows/sd15-base-frames-api.json`
- Video chain → local Wan 2.2 via ComfyUI, then `local-ffmpeg` fallback
- Voice → VibeVoice through ComfyUI; eSpeak-NG remains available as a local fallback
- Subtitles → bundled `whisper-cli` plus Homebrew Whisper/FFmpeg/FFprobe

See [OSS_STACK.md](OSS_STACK.md) for the full per-stage technology choices.
See [VIDEO_BUILD_MESH.md](VIDEO_BUILD_MESH.md) for the connected design/motion/render workflow.
See [docs/COMFYUI_LOCAL.md](docs/COMFYUI_LOCAL.md) for **local ComfyUI** host, port, and `models` path.
See [docs/BYTEPLUS_ARK.md](docs/BYTEPLUS_ARK.md) for optional **BytePlus ModelArk** (OpenAI-compatible LLM).
...
# 5. Verify the configured pipeline
.venv/bin/python tools/pipeline_doctor.py

# ComfyUI port/model paths differ per machine → see docs/COMFYUI_LOCAL.md
# Optional heavy check: also render a tiny local Wan video smoke
.venv/bin/python tools/pipeline_doctor.py --wan-smoke

# 6. Render the Remotion composition
npm install
npm run build
```
...
The cross-stage video build mesh lives in `dashboard/engines/video_build_mesh.py`.
It keeps HyperFrames, Open Design, UI/UX Pro Max, Remotion compatibility, and
QA checks attached to each build artifact so the tools work together instead of
becoming separate one-off helpers.
...
├── VIDEO_BUILD_MESH.md    # Connected UI/design/motion/render/QA workflow
├── README.md              # This file
